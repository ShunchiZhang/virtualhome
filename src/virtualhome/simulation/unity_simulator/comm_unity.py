...
class UnityCommunication(object):
    ...
    def __init__(
        ...
        if file_name is not None:
            self.launcher = communication.UnityLauncher(
                port=port,
                file_name=file_name,
                x_display=x_display,
        ...
        try:
            if repeat:
                resp = self.requests_retry_session().post(
                    self._address, json=request_dict
                )
            else:
                resp = requests.post(
    ...
    def add_character_camera(
        self, position=[0, 1, 0], rotation=[0, 0, 0], name="new_camera"
    ):
        """
    ...
    def fast_reset(self):
        response = self.post_command(
            {"id": str(time.time()), "action": "fast_reset", "intParams": []}
        )
        return response["success"]
    ...
    def camera_image(
        self, camera_indexes, mode="normal", image_width=640, image_height=480
    ):
        """
...
def _decode_image(img_string):
    ...
    if "PNG" == img_bytes[1:4]:
        ...
        img_file = cv2.imdecode(
            np.fromstring(img_bytes, np.uint8),
...
class UnityEngineException(Exception):
    ...
    def __init__(self, status_code, resp_dict):
        resp_msg = (
            resp_dict["message"] if "message" in resp_dict else "Message not available"
        )
        self.message = (
            "Unity returned response with status: {0} ({1}), message: {2}".format(
